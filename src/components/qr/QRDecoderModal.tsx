      ...
      <div className="neu-card rounded-major max-w-md w-full p-6 space-y-4 border border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Icon icon="solar:camera-minimalistic-outline" className="h-5 w-5 text-cyan-accent" />
        ...
        <div className="flex gap-2 bg-background p-1 rounded-control text-xs">
          <button type="button" onClick={() => { stopCamera(); setTab('upload'); }} className={`flex-1 py-1.5 font-semibold rounded-lg ${tab === 'upload' ? 'bg-primary text-white' : 'text-text-muted'}`}>Upload Image</button>
          <button type="button" onClick={() => { setTab('camera'); startCamera(); }} className={`flex-1 py-1.5 font-semibold rounded-lg ${tab === 'camera' ? 'bg-primary text-white' : 'text-text-muted'}`}>WebCam</button>
        </div>
        ...
        {tab === 'camera' && (
          <div className="space-y-2">
            <div className="relative aspect-video w-full rounded-card overflow-hidden bg-black flex items-center justify-center">
              ...
              {scanning && <div className="absolute inset-0 border-2 border-cyan-accent/50 animate-pulse flex items-center justify-center"><div className="w-40 h-40 border-2 border-cyan-accent rounded-xl" /></div>}
            </div>
        ...
        {error && <div className="p-2.5 rounded-control bg-red-500/10 text-red-500 text-xs">{error}</div>}

        {result && (
          <div className="rounded-card neu-card p-3.5 space-y-2.5 border-l-4 border-l-green-accent">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => { onApplyDecodedText(result.text); stopCamera(); onClose(); }} className="flex-1 bg-primary text-white py-1.5 text-xs font-semibold rounded-control shadow-glow-indigo">Load in Editor</button>
            </div>
          </div>
        )}
      </div>
